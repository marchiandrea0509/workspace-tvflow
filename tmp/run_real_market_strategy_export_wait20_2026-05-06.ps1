...
$replyTo = '1501615872090050623'
$symbolsFile = 'C:\Users\anmar\.openclaw\workspace-tvflow\tmp\tradingview_real_market_stocks_watchlist_symbols_2026-05-06.txt'
...
$outDir = Join-Path $reportsRoot "real_market_stocks_watchlist_wait20_$stamp"
$retryDir = Join-Path $reportsRoot "real_market_stocks_watchlist_wait20_retry_$stamp"
$zipPath = Join-Path $zipRoot "tradingview_real_market_stocks_watchlist_strategy_test_wait20_$stamp.zip"
...
function Invoke-ExportRun {
  ...
  & node $exportScript --watchlist tradingview_real_market_stocks_watchlist --exchange BATS --outdir $OutputDir --export-attempts 5 --symbols-file $SymbolsPath --post-export-wait-ms 20000
...
$requested = @(Get-Content $symbolsFile | Where-Object { $_.Trim() })
$bad = @($requested | Where-Object { $_ -match 'USDT|\.P|BITGET' })
if ($bad.Count -gt 0) { throw "Refusing to run: non-stock/BITGET-like symbols in real-market list: $($bad -join ', ')" }
...
$bitgetLeak = @($manifest.requestedSymbols | Where-Object { $_ -match 'USDT|\.P|BITGET' })
if ($manifest.watchlist -ne 'tradingview_real_market_stocks_watchlist') { throw "Wrong watchlist label in manifest: $($manifest.watchlist)" }
if ($bitgetLeak.Count -gt 0) { throw "Refusing to zip/send: BITGET-like symbols detected in manifest: $($bitgetLeak -join ', ')" }

# Keep the attachment clean: final CSVs + manifests + any failure screenshots only.
$packageDir = Join-Path $outDir '_zip_package'
if (Test-Path $packageDir) { Remove-Item $packageDir -Recurse -Force }
New-Item -ItemType Directory -Force -Path $packageDir | Out-Null
Get-ChildItem $outDir -File | Where-Object { $_.Name -match '(\.csv$|^manifest\.json$|^retry_manifest\.json$|_failure\.png$)' } | ForEach-Object {
...
$summary = if ($failedFinal.Count -gt 0) {
  "$($manifest.okCount) charts exported, $($manifest.failCount) failed: $($failedFinal -join ', '). Watchlist=tradingview_real_market_stocks_watchlist, exchange=BATS, wait=20s."
} else {
  "$($manifest.okCount) charts exported, 0 failed. Watchlist=tradingview_real_market_stocks_watchlist, exchange=BATS, wait=20s."
}
...
[pscustomobject]@{
  ...
  requestedSymbols = $manifest.requestedSymbols
