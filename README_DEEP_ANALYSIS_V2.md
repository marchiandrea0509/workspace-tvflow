...
- Input-integrity check: before treating screenshots as primary chart truth, the workflow must check screenshot usability. Mechanical packet checks cover missing files, tiny/corrupt images, low resolution, unusual aspect ratio/squeezed captures, and contact-sheet fallback; the final screenshot read must also check semantic clarity: correct symbol/timeframe, readable price/time axes, visible indicators/panes, no cropped/blurred/half-loaded chart. Every report must include a compact `Input screenshot audit` section: one PASS/no-material-issue row when screenshots are usable, plus detailed rows only when issues materially affect analysis confidence, level precision, or ticket validity.
...
- TP design is structure-first and independently optimized per order leg. Build a ranked TP candidate map before rejecting A/B/C for poor R:R. Each order has one TP; distinct TPs are preferred, but identical TPs are allowed when the same structural target is objectively the best realistic exit for multiple legs. Do not invent farther/fantasy TPs just to make TP values different. Poor-R:R rejections must include the TP candidates tested, best realistic per-leg assignment, per-leg R:R, all-filled R:R, and whether the rejection is genuinely poor geometry or insufficient open space.
- A validated report is still wrong if it suppresses valid A/B ladder alternatives into a single conservative leg. When A and/or B are chart-valid but live liquidity/orderability is RED, preserve the full ticket(s), mark them `WATCH_ONLY / NOT_LIVE_PLACEABLE`, and show liquidity as the blocker instead of deleting the ladder.
...
- Delivery-auditor hard gate: after `scripts/finalize_deep_analysis_delivery.py` passes, run the deterministic audit command `python scripts\audit_deep_analysis_delivery.py --report reports\deep_analysis\<REPORT>.md` before sending the Discord answer. The deterministic finalizer+auditor is the normal mandatory gate. Do **not** spawn/wait for the optional delivery-auditor subagent after deterministic `AUDIT_PASS`; reserve that subagent for debugging an `AUDIT_FAIL`, repeated formatting regressions, or explicit QA requests. The auditor is deliberately narrow: it checks clean user-visible chunks, complete 5-day swing-plan sections, canonical ticket/orderability/risk tables, no internal marker leakage, contiguous chunks, manifest consistency, and final verdict presence. It must print `AUDIT_PASS`; on `AUDIT_FAIL`, fix the report/chunks and rerun finalizer/auditor before replying.
...
Discord/reporting rule: final chat replies must explicitly **read/analyze all three screenshots: 1D, 4H, and 1H**. The Chart Context Read must include distinct 1D / 4H / 1H interpretations; if one screenshot is missing, unclear, or not used, say so explicitly. 1H is mandatory for tactical refinement, shallow-entry validation, compression/rejection, and chase-risk assessment; if it does not change the 4H plan, say: `1H does not change the 4H plan.` For speed, prefer a single multi-image model call containing the three original screenshots labeled `1D`, `4H`, and `1H`; this is allowed only as batching, not evidence reduction. If multi-image reading is unavailable or any timeframe interpretation is ambiguous, fall back to separate image calls. Delivery is separate from analysis: release the analysis text first with no screenshots attached, then send full-resolution original screenshots as separate follow-up messages, one image per message, **4H first and 1D second only**. Do not send 1H screenshots unless explicitly requested. The builder exposes `discord_media_delivery_mode` and `discord_media_messages` for this sequence. Merged `1D | 4H | 1H` contact sheets are emergency fallback/preview evidence only.

Output completeness rule: every deep-analysis chat/report must print the full required section order from the active master prompt, using the compact 5-day swing-plan style when possible: header/classification, Context and State, Input screenshot audit, Key Levels, Pullback Impulse Used, A/B/C/D options, Orderability traffic-light table, Risk Sizing/Tickets, and Final Verdict. Do not collapse the chat answer into only a chart read plus verdict or only actionable ticket levels. If the full report is saved but the chat omits input screenshot audit, impulse, traffic lights, or valid B/C ticket tables, treat the workflow as failed and correct it before replying.

Validator limitation / chat-parity rule: `validate_deep_analysis_report.py` only proves the saved markdown contains the required section family. It does **not** prove the Discord answer was acceptable. If the chat reply is a compressed `WAIT / WATCH_ONLY` summary while the user expected the 5-day swing-plan/ticket format, that is still a regression even when the saved report passes validation.
...
reports/deep_analysis_packets_v2/YYYYMMDD_HHMMSS_SYMBOL/
  ...
  derived/
    ...
    screenshot_integrity_check.json    # mechanical screenshot presence/dimension/readability precheck
...
- Fixed TP/SL per order at creation time. Each ladder leg must have one independently assigned structural TP. Identical TPs are allowed when the same realistic structural target is best for multiple legs; do not force different targets or reduce a valid ladder only because two legs share the same realistic TP.
...
- Manual visible-SL audit override: when the static scan rejects a pullback because its own chosen SL is too wide/too close to stale lower structure, manually test the visible-chart candidate SL from the current thesis before rejecting A/B. For longs, a hard SL below the immediate pre-breakout / retest shelf can be structural even when older supports or EMA200 sit lower, if a 4H loss of that shelf invalidates the pullback thesis. `sl_hierarchy_uncleared_levels` are warnings, not automatic vetoes. Regression case: ASMLUSDT 2026-06-04 - GPT's coherent SL 1625 below the 1633 shelf should have been audited before OC rejected A/B based on a builder-selected ~1574 SL.
...
- 3-6 realistic structural TP candidates.
- Several realistic per-leg TP assignments, normally L1/S1 -> T1, L2/S2 -> T1 or T2, L3/S3 -> T2 or T3. Choose the nearest credible assignment that passes, not the farthest/most optimistic one.
- For each candidate/assignment: L1-only R:R, L1+L2 R:R, all-filled R:R, planned risk, rounded quantity, estimated margin, per-leg 4H ATR distance to TP, selected leverage, and estimated liquidation-vs-SL safety. R:R is calculated per leg using that leg's own TP; all-filled R:R uses summed per-leg expected profit divided by total planned risk.
...
- TP checks are per-leg first: normal `1.0-3.0 ATR4H`, extended `>3.0-3.5 ATR4H`, aggressive `>3.5 ATR4H` only with strong 4H/1D structure and open space. Blended TP distance may be reported as context only; it is not the main realism gate.
...
The scan chooses the best valid static ticket, not the most optimistic one. Tighter SLs are allowed only beyond real support/invalidation and outside normal 4H noise. Far TP targets must be structurally meaningful. Do not reject based only on the nearest conservative TP if a realistic next structural TP exists; if no assignment passes, print the TP rejection audit from the master prompt.

Breakout/breakdown C scan: generate T1/T2/T3 from visible open space and structure. Required: trigger->T1 open space about `1.2 ATR4H` minimum, preferred trigger->selected TP about `1.5-2.5 ATR4H`, reject when major 1D SR blocks before T1, allow T2/T3 only when BO/BD structure and open space support them, and never use fantasy fib/ATR extensions without visible structure. For VOCO/C100, total reward must use the selected per-leg TP assignment; C100 all-filled aggregate R:R = summed expected profits / total all-filled-to-SL risk.
