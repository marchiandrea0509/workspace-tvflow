...
- Deep analysis must treat the user-selected screener symbol as preselected context only; remove any strict `score >=70` eligibility rule. Target planned risk remains **100 USDT**, but if structure, R:R, freshness, or the **500 USDT max margin** cap at planned leverage make the ticket weak/invalid, return a strong warning or WAIT/NO_TRADE rather than silently weakening the standard target.
- The `500` cap in deep-analysis tickets means **max margin at planned leverage**, not max total notional.
- Every generated trade ticket must perform and show a selected-leverage liquidation-vs-SL sanity check; if liquidation is not safely beyond SL, return WAIT/NO_TRADE or a hard warning instead of treating the ticket as valid.
...
- Default live-order liquidity fallback rule: when a live Bitget order request fails the liquidity/slippage gate, immediately provide two downsized proposals instead of only rejecting: (1) cap extra stop-market slippage cost to `slippage_pct × original/base planned risk` (default `slippage_pct = 5%`, base risk currently normally `100 USDT`, so default max extra slippage `5 USDT`), and (2) cap extra stop-market slippage cost to `slippage_pct × new planned no-slippage risk`. Keep `slippage_pct` parameterized because Andrea may change base risk or tolerance later. For each proposal, solve size from the live orderbook stop-exit depth, round down to symbol precision, show planned no-slippage risk, estimated extra slippage, total risk after slippage, notional/margin, and whether volume/depth gates still pass or remain caution/no-go.
- The current screener version context for new deep-analysis work is `OC Hybrid Edge Screener v11.6.x`.
- Simple deterministic deep-analysis support tasks should be delegated to cheap mini/nano subagents on Codex OAuth/local zero-cost routes, not paid API keys. `tvflow` keeps coordination, final trade judgment, and live-execution confirmation boundaries.
- Ladder generation is a critical quality surface: it must consider R:R and meaningful structural levels. Do not reject an LC/DIP ladder merely because current price is near resistance or RSI is high; those can support the pullback thesis. A no-ladder decision needs stronger reasons such as CHoCH, degraded trend, high SL-hit probability, stale data, liquidity/fee issue, or objectively poor R:R. Deep structural L3 levels are valid when they remain plausible for the setup family/timeframe and do not imply trend failure.
