...
def recent_swings(rows, look=3, n=8):
    ...
    return {'highs': [{'i':i,'price':p,'dt':dt} for i,p,dt in highs[-n:]], 'lows': [{'i':i,'price':p,'dt':dt} for i,p,dt in lows[-n:]]}
...
def snap(rows):
    ...
    vol20=statistics.mean([x['base_volume'] for x in last20]) if last20 else 0
    qvol20=statistics.mean([x['quote_volume'] for x in last20]) if last20 else 0
    return {
        'last':r,
        ...
        'last_volume_vs_sma20': r['base_volume']/vol20 if vol20 else None,
        'recent_swings': recent_swings(rows[-120:],3,8),
    }


def find_latest_strategy_csv():
    roots=[Path(r'C:\Users\anmar\.openclaw\workspace\tradingview\reports\strategy_test_watchlist_csv'), Path('reports')]
    candidates=[]
    for root in roots:
        if root.exists(): candidates += list(root.glob(f'**/{TV_SYMBOL}_strategy_test_4h.csv')) + list(root.glob(f'**/*{SYMBOL}*strategy*4h*.csv'))
...
def find_latest_screener_row():
    roots=[Path(r'C:\Users\anmar\.openclaw\workspace\tradingview\reports\pine_screener'), Path('reports/pine_screener')]
    files=[]
    for root in roots:
        if root.exists(): files += list(root.glob('*.csv'))
    for p in sorted(set(files), key=lambda p:p.stat().st_mtime, reverse=True)[:80]:
        try:
            with p.open(encoding='utf-8-sig', newline='') as f:
                rows=list(csv.DictReader(f))
        except Exception:
