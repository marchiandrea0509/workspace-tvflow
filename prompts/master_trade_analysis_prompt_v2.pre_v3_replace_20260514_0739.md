...
- 4H: analyze the last **40 candles** first.
- 1H: analyze the last **80 candles** first.
- 1D: analyze the last **80 candles** first.
- Older data is allowed only for major HTF levels, not for forcing stale pivots into the active ladder.
...
- Visible chart structure first, context second, raw OHLCV validation third.
- Do not choose old pivots just because they are confirmed; fresh breakout highs/lows and current visible structure matter more for active pullback geometry.
- Do not ignore fresh breakout highs/lows when the chart clearly shows an active impulse.
- Do not reject limit ladders only because price is currently hot; hot price blocks market chase entries, not valid resting pullback limits.
...
- Always show the two-option framework, even when only one option is valid:
...
Allowed styles: `AUTO`, `SINGLE_LIMIT_PULLBACK`, `DIP_LADDER`, `BREAKOUT`, `SELL_RALLY`, `BREAKDOWN`, `WAIT`.
...
- If only one valid pullback level survives all checks, it may be `AUTO / SINGLE_LIMIT_PULLBACK`, not a forced/invalid ladder.
...
1. Latest valid 4H bullish impulse:
   ...
   - Fresh breakout exception: if live/current price or latest 4H/1H high is >= 0.75 × ATR4H above the latest confirmed 4H pivot high and 4H trend is bullish/bullish-neutral, use `ACTIVE_BREAKOUT_UNCONFIRMED_HIGH`: active high = max(current, latest 4H high, latest 1H high), active low = latest meaningful 4H/1H pullback low/support base before the breakout. Do not wait for a confirmed pivot high.
2. Value zone:
...
- fresh breakdown exception: use `ACTIVE_BREAKOUT_UNCONFIRMED_LOW` when live/latest lows break at least 0.75 × ATR4H below the latest confirmed 4H pivot low in bearish/bearish-neutral context;
...
Reject ladder if:
- price is at/near major resistance for long or major support for short **and the plan is a market/chase entry**; for resting pullback limits this is a warning, not a hard reject, if static checks pass;
...
### 1) Screenshot / visible chart read first

Start from the supplied 1D and 4H screenshots/contact sheet when available. Summarize:
- 1D visible macro structure over the last 80 candles;
- 4H visible execution structure over the last 40 candles;
- 1H tactical timing over the last 80 candles when supplied in the packet;
- visible TF alignment/conflict;
- any conflict between screenshot structure and OHLCV/packet summaries.
...
```json
{
  ...
  "options": [
    {
      ...
      "status": "VALID_or_REJECTED",
      "rejected_levels": [],
      "key_failed_metric": null,
      "reason": "exact reason if rejected or why valid"
    },
    {
      "name": "Option B — BEST FILL PROBABILITY",
      "status": "VALID_or_REJECTED",
      "rejected_levels": [],
      "key_failed_metric": null,
      "reason": "exact reason if rejected or why valid"
  ...
  "shallow_fill_probability_leg_audit": [
    {
      "candidate_entry": 0,
      "sl": 0,
      "nearest_tp": 0,
      ...
      "reason": "exact accepted/rejected reason"
    }
  ],
  "rejection_audit": [
    {
