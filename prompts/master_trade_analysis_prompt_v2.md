...
- Always include a compact square markdown liquidity/orderability gate table when live gate data is available. Use numeric observed values, numeric limits/required values, and traffic-light emojis in every row. Required columns exactly: `Gate | Observed value | Limit / required | Light | Note`. Include at least: spread, 24h quote volume, dead 1m candles, p10 1m quote-volume / RWA active-session volume stress, stop-exit simulated slippage, depth-to-SL, existing orders/position, margin/leverage if relevant, and confirmation boundary. If a gate was not run or not applicable, say `not run` / `n/a`; do not write vague `Orderability: OK` without the square gate table.
