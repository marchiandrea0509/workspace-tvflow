...
- Before using TradingView screenshots as primary chart truth, check whether the input is actually usable: correct symbol/timeframe, full 1D/4H/1H coverage, readable price/time axes, enough resolution, no squeezed/cropped chart, no missing/hidden important indicator panes, no blurry/overlapped labels, and no obvious half-loaded/blank capture.
- Always include a compact `Input screenshot audit` section/table after `Context and State` and before `Detected Level Map`. If screenshots are clear enough, print one PASS row such as `All 1D/4H/1H screenshots | PASS / no material issue | Axes, resolution, and panes usable for analysis`. Add detailed rows only for issues that materially affect analysis confidence, level precision, or ticket validity.
- If a screenshot issue affects only one timeframe, reduce confidence or use OHLCV/export fallback only for that affected part; do not pretend a precise visual level is visible when it is not.
- In the fast JSON path, `inputIntegrity` is required. Include at least one row with `source`, `issue`, and `analysisImpact`: either PASS/no material issue or the material limitation(s).
...
- Chat-output gate: after the saved report exists, run the strict delivery finalizer `python scripts\finalize_deep_analysis_delivery.py --report <report.md>` and send the generated `discord_chunks/chunk_*.md` files verbatim, in order, not a hand-written compressed summary. This is mandatory for Discord deep-analysis delivery after the 2026-06-17 GEUSDT regression. If the finalizer fails, stop and fix the report; do not answer with a summary. The older `render_deep_analysis_chat_reply.py` command is debugging support only. If a final assistant answer would omit Header/classification, Context and State, Input screenshot audit, Detected Level Map, Pullback Impulse Used, A/B/C1/C2/D sections, Orderability traffic lights, Risk sizing, and Final verdict, it is a failed deliverable even when the saved report passes.
...
- ATH / price-discovery refresh audit: after a fresh ATH breakout, do not reject all higher refresh ladders merely because no historical resistance exists above price. Build a conservative extension TP map from visible breakout geometry: fresh high retest, 1.118/1.272 impulse extensions, and nearby round-number front-runs (for example `366`, `368.5` before `370`). These are valid static TP candidates when they are conservative, below the next obvious round-number magnet, and paired with a structural shelf-loss SL. Regression case: GEUSDT 2026-06-17 — GPT Option B used `334.54→362.08`, entries `356.20/352.60/348.80`, SL `345.90`, TPs `366.00/366.00/368.50`; tvflow overfiltered by using a deeper `339.81` SL and capping/rejecting extension TPs.
- User/GPT exact-ticket audit: when Andrea supplies a preferred GPT/user ticket, calculate that exact ticket before final rejection: leg risks, leg R:R, first-leg R:R, first-2-leg R:R, all-filled R:R, margin at requested/implied leverage, and live liquidity. Do not let the packet static scan or `sl_hierarchy_uncleared_levels` veto the ticket before testing the supplied SL/TP combination. If the exact ticket is chart-valid but liquidity is RED, preserve it as `WATCH_ONLY / REQUIRES RED-LIQUIDITY OVERRIDE`, not as an omitted candidate.
- Huge-impulse prior-high retest audit: after a very large 4H impulse from a broad base through a previous-week/range high into a fresh high, test both (a) the tactical flag shelf SL and (b) the deeper prior-high retest SL below the broken range high before choosing A/B. Do not omit the deep prior-high retest leg merely because it is several ATR below current price when it is still inside the active 5-day breakout thesis and produces the main aggregate R:R. A shallow B1 with ~1R or slightly below can be valid only if its risk share is small, deeper legs carry the quality, and aggregate all-filled R:R passes. Regression example: ASMLUSDT 2026-06-14 GPT B used broad 1620.35→1926.68, entries 1846.5/1821.0/1779.0, SL 1768.0, TP 1924.5; tvflow over-focused on 1805.5 tactical invalidation and omitted the 1779 retest leg.
