...
- Chat-output gate: after the saved report passes, render the actual Discord/chat answer with `python scripts\render_deep_analysis_chat_reply.py --report <report.md> --out <reply.md> --chunk-dir <reply_chunks_dir>` and send the generated chunk files verbatim, in order, not a hand-written compressed summary. The renderer must fail closed for long Discord replies without `--chunk-dir`; do not bypass that failure by summarizing. If a final assistant answer would omit Header/classification, Context and State, Detected Level Map, Pullback Impulse Used, A/B/C1/C2/D sections, Orderability traffic lights, Risk sizing, and Final verdict, it is a failed deliverable even when the saved report passes.
