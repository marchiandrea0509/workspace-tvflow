...
async function adaptTimeAxis(page, timeframe, width, height, options = {}) {
  ...
  try {
    ...
    if (!options.skipHorizontalZoom) {
      // Deep-analysis screenshots must be readable in Discord.  The old positive
      // wheel motion zoomed the time axis out, leaving tiny candles and lots of
      // empty chart space.  Use a small zoom-in instead, especially on 4H.
      const zoomInSteps = timeframe === '1D' ? 3 : (timeframe === '4H' ? 4 : 2);
...
async function hideObstructiveUi(page) {
  try {
    await page.evaluate(() => {
      const textNeedles = ['crypto sale', 'limited time offer', 'upgrade now', 'advertisement', 'press and hold to see detailed chart values'];
      const candidates = Array.from(document.querySelectorAll('[role="dialog"], [class*="dialog"], [class*="modal"], [class*="popup"], [data-name*="dialog"], [data-name*="promo"], iframe'));
      for (const el of Array.from(document.querySelectorAll('body *'))) {
        try {
          const cs = window.getComputedStyle(el);
          const z = Number(cs.zIndex || 0);
          const txt = String(el.innerText || el.textContent || '').toLowerCase();
          if ((cs.position === 'fixed' || cs.position === 'sticky') && z >= 50 && textNeedles.some((n) => txt.includes(n))) {
            candidates.push(el);
          }
        } catch {}
      }
      for (const el of candidates) {
        try {
          ...
          const cs = window.getComputedStyle(el);
          const z = Number(cs.zIndex || 0);
          const isDialog = el.getAttribute('role') === 'dialog' || /dialog|modal|popup|promo/i.test(el.className || '') || /dialog|promo/i.test(el.getAttribute('data-name') || '');
          const isAdLike = textNeedles.some((n) => txt.includes(n));
          const isHighOverlay = (cs.position === 'fixed' || cs.position === 'sticky') && z >= 50;
          // TradingView can show an instructional tooltip directly over the chart; remove it even
          // when it is not implemented as a normal dialog/high-z overlay.
          if (isDialog || isAdLike || isHighOverlay) el.remove();
        } catch {}
      }
      for (const el of Array.from(document.querySelectorAll('body *'))) {
        try {
...
async function expandIndicatorPanes(page, width, height) {
  try {
    let panes = await getPaneRects(page);
    if (panes.length < 2) return false;
    const firstIndicator = panes[1];
    const bottom = panes[panes.length - 1].bottom;
    const totalPaneHeight = Math.max(1, bottom - panes[0].top);
    const indicatorHeight = Math.max(0, bottom - firstIndicator.top);
    // If indicators are collapsed/tiny, drag the divider upward so the full
    // Openclaw-structure evidence pane is readable in Discord screenshots.
    if (indicatorHeight / totalPaneHeight >= 0.22 && firstIndicator.height >= 180) return false;
    const targetIndicatorHeight = Math.min(Math.max(Math.floor(totalPaneHeight * 0.28), 260), Math.floor(height * 0.36));
    const targetTop = Math.max(panes[0].top + Math.floor(totalPaneHeight * 0.52), bottom - targetIndicatorHeight);
    const x = Math.floor(firstIndicator.left + firstIndicator.width * 0.5);
    const y = Math.floor(firstIndicator.top + 2);
    await page.mouse.move(x, y);
    await page.mouse.down();
    await page.mouse.move(x, targetTop, { steps: 18 });
    await page.mouse.up();
    await page.waitForTimeout(700);
    return true;
  } catch {
...
(async () => {
  ...
  const mainPaneOnly = String(args.mainPaneOnly || 'false').toLowerCase() === 'true';
  const focusRecent = String(args.focusRecent || 'true').toLowerCase() !== 'false';
  ...
  const fileSymbol = String(args.fileSymbol || symbol).replace(/[^a-z0-9._-]+/gi, '-');
  const suffix = layout ? `_${layout.replace(/[^a-z0-9-_]+/gi, '-')}` : '';
  const baseShotPath = path.join(outdir, `${fileSymbol}_${timeframe}${suffix}`);
  ...
  try {
    ...
    const launchArgs = headless
      ? ['--disable-gpu', '--disable-gpu-compositing', '--use-gl=swiftshader', '--use-angle=swiftshader', '--disable-dev-shm-usage']
      : ['--enable-gpu', '--use-angle=d3d11', '--disable-dev-shm-usage'];
    ...
    if (!mainPaneOnly) {
      await expandIndicatorPanes(page, width, height);
      await page.waitForTimeout(250);
    }
    ...
    if (mainPaneOnly) {
      ...
      const main = panes[0];
      if (main) {
        const focusOffset = focusRecent ? Math.floor(main.width * 0.08) : 0;
        const clipX = Math.max(0, Math.floor(main.left + focusOffset));
        const clipY = Math.max(0, Math.floor(main.top - 44));
        // Include extra right margin so the TradingView price axis is not clipped.
        const clipW = Math.min(width - clipX, Math.floor(main.width - focusOffset + 180));
        const clipH = Math.min(height - clipY, Math.floor(main.height + 72));
        await page.screenshot({ path: shotPath, fullPage: false, clip: { x: clipX, y: clipY, width: clipW, height: clipH } });
      } else {
    ...
    fs.appendFileSync(
      logPath,
      `[live-ok] symbol=${symbol} timeframe=${timeframe} layout=${layout || 'default'} preset=${preset || 'standard'} profile=${profileName} width=${width} height=${height} scale=${scale} headless=${headless} fixedZoom=${fixedZoom} mainPaneOnly=${mainPaneOnly} focusRecent=${focusRecent} resetScales=${resetScales} chartReady=${ready} collapsedRightPanel=${collapsedPanel} panelShot=${panelTaken} url=${url} file=${shotPath}${panelTaken ? ` panelFile=${panelPath}` : ''} metaFile=${metaPath} price=${market.price ?? 'na'} changePct=${market.changePct ?? 'na'}\n`
    );
