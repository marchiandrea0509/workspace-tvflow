...
def snap(rows):
    ...
    return {
        ...
        'piv_res': sorted(set(round(x,4) for x in above))[:8],
        'piv_sup': sorted(set(round(x,4) for x in below), reverse=True)[:8],
...
def read_latest_strategy_csv():
    candidates=sorted(Path(r'C:\Users\anmar\.openclaw\workspace\tradingview\reports\strategy_test_watchlist_csv').glob('**/AAPLUSDT.P_strategy_test_4h.csv'), key=lambda p:p.stat().st_mtime, reverse=True)
    if not candidates: return None
    p=candidates[0]
    with p.open(encoding='utf-8-sig', newline='') as f:
        rows=list(csv.DictReader(f))
    # choose last row with non-empty score values
    good=[r for r in rows if r.get('02 Best Score') not in ('',None)]
    last=good[-1] if good else rows[-1]
    ...
    return {'path':str(p),'rows':len(rows),'last':{k:last.get(k) for k in fields if k in last}}


def read_screener_row():
    p=Path(r'C:\Users\anmar\.openclaw\workspace\tradingview\reports\pine_screener\pine_screener_2026-05-01T12-08-35-838Z.csv')
    if not p.exists(): return None
    with p.open(encoding='utf-8-sig', newline='') as f:
        rows=list(csv.DictReader(f))
    row=next((r for r in rows if r.get('Symbol')==TV_SYMBOL), None)
    return {'path':str(p), 'row':row}

rows4=enrich(candles('4H',300)); rows1=enrich(candles('1D',300))
...
for gran, rows in [('4H',rows4),('1D',rows1)]:
    path=OUT/f'2026-05-05_AAPLUSDT_{gran}_bitget_ohlcv.csv'
    with path.open('w', newline='', encoding='utf-8') as f:
        w=csv.DictWriter(f, fieldnames=list(rows[0].keys())); w.writeheader(); w.writerows(rows)
summary={'generated_utc':datetime.now(timezone.utc).isoformat(),'symbol':SYMBOL,'tv_symbol':TV_SYMBOL,'ticker':ticker,'funding':fund,'open_interest':oi,'contract':contract,'snap4h':snap(rows4),'snap1d':snap(rows1),'strategy_csv':read_latest_strategy_csv(),'screener_row':read_screener_row()}
(OUT/'2026-05-05_AAPLUSDT_metrics.json').write_text(json.dumps(summary,indent=2),encoding='utf-8')
